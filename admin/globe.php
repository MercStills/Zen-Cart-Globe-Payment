...
<div id="globe_accounts" style="margin-left: 5%;">
<?php 
    	require_once '../includes/modules/payment/globe/jsonRPCClient.php';

		$globe = new jsonRPCClient('http://'.MODULE_PAYMENT_GLOBE_LOGIN.':'.MODULE_PAYMENT_GLOBE_PASSWORD.'@'.MODULE_PAYMENT_GLOBE_HOST.'/'); 
		
		?>
		
		<h1 class="pageheading">Globe Accounts</h1>
		<table border="0">
		...
		try {
			...
			echo 'Error: Globe server is down.  Please email system administrator.';
			$down = true;
		}
		if(!$down){
			global $db;
			$accounts = $globe->listaccounts();
			//print_r($accounts);
			$count = 0;
			foreach($accounts as $a=>$t){
				//if($a!==''){
					$bc = $globe->getaddressesbyaccount($a);
					print('<tr><td align="left">'.$a.'</td><td border="1px">');
					foreach($bc as $b){
						$v = $globe->getreceivedbyaddress($b);
						print($b.'<br />');
						...
						if ($result->RecordCount() > 0) {
						  while (!$result->EOF) {
						    ...
						    echo '<a href="'.zen_href_link('orders.php?page=1&oID='.$result->fields['orders_id'].'&action=edit', '', 'NONSSL').'">Order '.$result->fields['orders_id'].'</a> | Due '.$order->fields['order_total'].'GLB | Received '.$v.'GLB  | '.$result->fields['orders_status_name'].'<br />';
						    $result->MoveNext();
						  }
						}
